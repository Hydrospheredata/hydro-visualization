...
from utils.data_management import model_has_correct_embeddings_field, get_training_data_path
...
def supported():
    ...
    try:
        if not model_has_production_data(model.id):
    ...
    if not training_data_path and not embeddings_exist:
        return {"supported": False,
                "message": f"Upload training data to use projector for models without correct {EMBEDDING_FIELD} field. {EMBEDDING_FIELD} field should return vector of shape: [1, some_embedding_dim]."}
