    ...
    def make_input_dict(self, x, kwargs) -> Dict[str, np.array]:
        ...
        if type(x) is dict:
            ...
            raise NotImplementedError(
                "Implicit conversion of single np.array in multi-tensor contracts is not supported")
        elif x is None:
            kwargs = self.__check_for_python_builtins(kwargs)
            input_dict = kwargs
        else:
            raise ValueError(
                f"Conversion failed. Expected [pandas.DataFrame, numpy.array, pandas.Series, dict], got {type(x)}")
        return input_dict
